...
<script>
    ...
    window.location = "{{url('admin/set_session_all?lang=')}}"+lang+"&route={{route('the_ocs.create')}}";
  }

  function edit(para){
    var lang = para.substring(0,2);
    var id = para.substring(3);
    window.location = "{{url('admin/edit_session?lang=')}}"+lang+"&route={{url('admin/the_ocs/')}}"+"/"+id+"/edit";
  }
